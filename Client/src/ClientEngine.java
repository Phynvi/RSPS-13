...
public class ClientEngine extends Applet implements Runnable, ActionListener, MouseListener, MouseMotionListener, MouseWheelListener, KeyListener, FocusListener, WindowListener {
	...
	int anInt4,
		...
		readIndex,
		writeIndex;
	final long aLongArray7[] = new long[10];
	boolean shouldDebug;
	Graphics graphics;
...
//			popupMenu.add(menu);
//			if (image != null) {
//				final TrayIcon trayIcon = new TrayIcon(image, ClientConstants.name, popupMenu);
//				trayIcon.setImageAutoSize(true);
//				systemTray.add(trayIcon);
//				trayIcon.displayMessage(ClientConstants.name, (ClientConstants.name + " client has been started!"), MessageType.INFO);
//			}
//		}
	}
    ...
    public void createMenuBar() throws ClassNotFoundException, InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
		...
		final Menu fileMenu = new Menu(ClientConstants.name);
	...
	public final void exitOption() {
		int option = JOptionPane.showConfirmDialog(null, "Are you sure you want to exit " + ClientConstants.name + "?", "Confirm Exit", JOptionPane.YES_NO_OPTION);
		if (option == 0) {
			gameFrame.setTitle("Please wait, " + ClientConstants.name + " is exiting...");
			if (gameFrame != null) {
	            exit();
	            try {
	                Thread.sleep(1000L);
