public class CustomInterfaces extends RSInterface {
	...
	public static void accountManager(TextDrawingArea[] daniel) {
		...
		addText(56007, ("Having a safe account is crucial in the world of " + ClientConstants.name), daniel, 0, 0xff9933, true, true);
	...
	public static void setTPMenu(TextDrawingArea[] vencillio) {
		final int[] tpMenuIds = { 61000 };
		final int itemSize = 20;
		final int[] i0to2ids = { 2, 8, 8 },
				j0to2ids = { 11, 475, 475 },
				i3to16ids = { 35, 35, 35, 35, 35, 35, 35, 35, 61, 61, 61, 61, 9, 39 },
				j3to16ids = { 16, 16, 136, 136, 256, 256, 376, 376, 136, 136, 256, 256, 255, 75 },
				i17to28ids = { 39, 39, 39, 65, 65, 117, 140, 180, 200, 220, 250, 250 },
				j17to28ids = { 195, 315, 435, 195, 315, 110, 40, 40, 40, 40, 70, 70 },
				scrollSizes = { (10 * itemSize), (9 * itemSize), (5 * itemSize), 500, (7 * itemSize), (3 * itemSize) };
		for (int i = 0; i < tpMenuIds.length; i++) {
			RSInterface tab = addInterface(tpMenuIds[i]);
			addSprite((tpMenuIds[i] + 1), (63 + i));
			addHoverButton((tpMenuIds[i] + 2), 17, 21, 21, "Close", 250, (tpMenuIds[i] + 3), 3);
			addHoveredButton((tpMenuIds[i] + 3), 18, 21, 21, (tpMenuIds[i] + 4));
			for (int j = 0; j < 18; j += 3) {
				addHoverButton((tpMenuIds[i] + 5 + j), 59, 123, 30, "Select", 0, (tpMenuIds[i] + 6 + j), 1);
				addHoveredButton((tpMenuIds[i] + 6 + j), 60, 123, 30, (tpMenuIds[i] + 7 + j));
			}
			addText(61023, "@or2@Teleportation Menu", 0xff9933, true, true, -1, vencillio, 2);
			for (int j = 0; j < 7; j++)
				addText((tpMenuIds[i] + 24 + j), "", 0xff9933, true, true, -1, vencillio, ((j == 6) ? 2 : 1));
			for (int j = 0; j < 4; j++)
				addText((tpMenuIds[i] + 31 + j), "", 0xff9933, false, true, -1, vencillio, 0);
			addText((tpMenuIds[i] + 39), "", 0xff9933, false, true, -1, vencillio, 0);
			addHoverButton((tpMenuIds[i] + 35), 61, 123, 30, "Confirm Teleport", 0, (tpMenuIds[i] + 36), 1);
			addHoveredButton((tpMenuIds[i] + 36), 62, 123, 30, (tpMenuIds[i] + 37));
			addText((tpMenuIds[i] + 38), "Go!", 0xff9933, true, true, -1, vencillio, 3);
			tab.totalChildren(32);
			for (int j = 0; j < 3; j++)
				tab.child(j, (tpMenuIds[i] + 1 + j), j0to2ids[j], i0to2ids[j]);
			for (int j = 0, k = 0; j < 21; j += 3, k += 2) {
				tab.child((3 + k), (tpMenuIds[i] + 5 + j), j3to16ids[k], i3to16ids[k]);
				tab.child((4 + k), (tpMenuIds[i] + 6 + j), j3to16ids[k + 1], i3to16ids[k + 1]);
			}
			for (int j = 0; j < 12; j++)
				tab.child((17 + j), (tpMenuIds[i] + 25 + j), j17to28ids[j], i17to28ids[j]);
			tab.child(29, (tpMenuIds[i] + 38), 110, 257);
			tab.child(30, (tpMenuIds[i] + 50), -70, 109);
			tab.child(31, (tpMenuIds[i] + 39), 40, 160);
			RSInterface scrollInterface = addTabInterface(tpMenuIds[i] + 50);
			scrollInterface.width = 538;
			scrollInterface.height = 204;
			scrollInterface.scrollMax = scrollSizes[i];
			setChildren(40, scrollInterface);
			for (int j = 0, y = 3; j < 40; j++, y += itemSize) {
				addHoverText((tpMenuIds[i] + 51 + j), "", "Select", vencillio, 1, 0xff9633, true, true, 300);
				setBounds((tpMenuIds[i] + 51 + j), 260, y, j, scrollInterface);
			}
		}
	}
