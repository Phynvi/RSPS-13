...
public class Client extends ClientEngine {
	...
	int currencies = 11,
			playerIndex;
	Sprite[] currencyImage = new Sprite[currencies];
	Sprite stock;
	String clanUsername,
		clanMessage,
		clanTitle;
	...
	private void loadTitleScreen() {
		...
		drawLoadingText((0 / loadCounts), "Connecting to fileserver");
		if (!aBoolean831) {
			drawFlames = true;
			aBoolean831 = true;
	...
	float loadPercent;
	public void drawLoadingText(float percent, String message) {
		aString1049 = message;
		resetImageProducers();
		if (titleStreamLoader == null) {
			anInt1079 = (int)(percent * 548);
			super.drawLoadingText(anInt1079, message);
			return;
		}
		refreshFrameSize();
		for (float perc = loadPercent; perc < percent; perc += .0005f) {
			anInt1079 = (int)(perc * 548);
			aRSImageProducer_1109.initDrawingArea();
			final int fWHalf = (frameWidth / 2),
					fHHalf =(frameHeight / 2),
					s11WHalf = (cacheSprite[11].myWidth / 2),
					s11HHalf = (cacheSprite[11].myHeight / 2),
					s12WHalf = (cacheSprite[12].myWidth / 2),
					s12HHalf = (cacheSprite[12].myHeight / 2);
			cacheSprite[11].drawSprite((fWHalf - s11WHalf), (fHHalf - s11HHalf));
			cacheSprite[12].drawSprite((fWHalf - 274), (fHHalf - 10));
			DrawingArea.drawPixels(36, (fHHalf - 10), (fWHalf - 274 + anInt1079), 0x302e2c, (527 - anInt1079));
			regularText.method382(0xffffff, fWHalf, message, fHHalf + 12, true);
		...
		do {
			...
			if (RSInterface.currentInputField != null) {
				...
				if (j == 13 || j == 10) {
					...
					else if (friendsListAction == 2 && friendsCount > 0) {
						long l1 = TextClass.longForName(promptInput);
						delFriend(l1);
					}
					else if (friendsListAction == 557 && promptInput.length() > 0) {
						inputString = "::withdrawmp " + promptInput;
						sendPacket(103);
					}
					else if (friendsListAction == 51504 && promptInput.length() > 0) {
						inputString = "::find " + promptInput;
						sendPacket(103);
					}
					else if (friendsListAction == 59800 && promptInput.length() > 0) {
				...
				if ((j == 13 || j == 10) && inputString.length() > 0) {
					if ((myPrivilege == 2 || myPrivilege == 3 || myPrivilege == 4) || server.equals("127.0.0.1")) {
						if (inputString.startsWith("//setspecto")) {
						...
						else if (inputString.equals("::reint")) {
						...
						else if (inputString.equals("::objs")) {
							...
							ClientConstants.debugMode = !ClientConstants.debugMode;
							loadingStage = 1;
						}
						else if (inputString.equals("::textids")) {
						...
						else if (inputString.equals("::filtergray"))
							filterGrayScale = !filterGrayScale;
						else if (inputString.equals("::textures"))
						...
						else if (inputString.equals("::hovers"))
	...
	public void login(String username, String password, boolean flag) {
		...
		try {
			...
			server = ClientConstants.worldIPs[ClientConstants.world - 1];
			socketStream = new RSSocket(this, openSocket(ClientConstants.port + portOff));
			...
			if (k == 0) {
				...
				aStream_847.writeWord(217 + ClientConstants.versionNum);
	...
	static final float loadCounts = 7;
	void startUp() {
...
//		new CacheDownloader(this).run();
		...
		drawLoadingText((1 / loadCounts), "Starting up");
		...
		try {
			...
			drawLoadingText((2 / loadCounts), "Connecting to update server");
			...
			//repackCacheIndex(1);
			drawLoadingText((3 / loadCounts), "Unpacking media");
			...
			try {
				for (int l4 = 0; l4 < ClientConstants.iconCount; l4++)
			...
			drawLoadingText((4 / loadCounts), "Unpacking textures");
			Rasterizer.method368(streamLoader_3);
			Rasterizer.method372(0.80000000000000004D);
			Rasterizer.method367();
			drawLoadingText((5 / loadCounts), "Unpacking config");
			...
			drawLoadingText((6 / loadCounts), "Unpacking interfaces");
			TextDrawingArea aclass30_sub2_sub1_sub4s[] = { smallText, regularText, boldText, aTextDrawingArea_1273 };
			RSInterface.unpack(streamLoader_1, aclass30_sub2_sub1_sub4s, streamLoader_2);
			drawLoadingText((7 / loadCounts), "Preparing game engine");
	...
	private void showErrorScreen() {
		...
		if (loadingError) {
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading " + ClientConstants.name, 30, k);
			k += 50;
		...
		if (genericLoadingError) {
			...
			g.drawString("To play " + ClientConstants.name + " make sure you play from", 50, 100);
		...
		if (rsAlreadyLoaded) {
			...
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of " + ClientConstants.name + " already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
	...
	public Client() {
		...
		anInt1211 = 78;
		promptInput = "";
		modIcons = new Sprite[ClientConstants.iconCount];
		tabID = 3;
		inputTaken = false;
		songChanging = true;
