		...
		try {
			if (socket == null) {
				...
				if (l - openSocketTime < 4000L)
					return;
				openSocketTime = l;
				socket = clientInstance.openSocket(ClientConstants.port + Client.portOff);
	...
	public int method562(int i, int k, int l) {
		...
		for (int j1 = 0; j1 < mapIndices1.length; j1++)
			if (mapIndices1[j1] == i1)
				if (i == 0)
					return mapIndices2[j1] > 3535 ? -1 : mapIndices2[j1];
				else
					return mapIndices3[j1] > 3535 ? -1 : mapIndices3[j1];
		return -1;
	}
