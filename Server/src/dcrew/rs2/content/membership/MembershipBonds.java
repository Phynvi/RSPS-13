package dcrew.rs2.content.membership;
...
public class MembershipBonds {
	public enum BondData {
		...
		public int getSpent() { return moneySpent; }
		public int getCredits() { return credits; }
		public int getComplimentary() { return complimentary; }
	...
	public static boolean handle(Player player, int itemId) {
		...
		World.sendGlobalMessage("</col>[ @dre@" + Constants.name + " </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + "</col> has just reedemed a @dre@" + Utility.format(data.getCredits()) + "</col> credit voucher!");
