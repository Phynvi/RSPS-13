...
public class TradeContainer extends ItemContainer {
	...
	public void offer(int id, int amount, int slot) {
		...
		if (!trade.player.getInventory().slotContainsItem(slot, id))
			return;
		if ((trade.getState() == Trade.States.Confirmation) || (trade.getState() == Trade.States.ConfirmationAccepted))
			return;
		...
		trade.setState(Trade.States.Trade);
		trade.tradingWith.setState(Trade.States.Trade);
	...
	@Override
	public void update() {
		if ((trade.state == Trade.States.Confirmation) || (trade.state == Trade.States.ConfirmationAccepted)) {
